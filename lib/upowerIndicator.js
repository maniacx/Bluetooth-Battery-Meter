...
export const UpowerDevice = GObject.registerClass({
    ...
    _sync() {
        ...
        const isValidBatteryState = UPower.DeviceState.UNKNOWN !== this._proxy.get_cached_property('State').unpack();
        ...
        if (isPresent && showIndicator && isValidBatteryState && !indicator) {
        ...
        } else if (!isPresent || !showIndicator || !isValidBatteryState && indicator) {
            indicator?.destory();
            indicator = null;
