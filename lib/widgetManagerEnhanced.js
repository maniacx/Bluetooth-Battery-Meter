...
export const WidgetManagerEnhanced = GObject.registerClass({
    ...
    _syncConfigFlags() {
        ...
        this.useBluezBattery = config.useBluezBattery;
        this.alwaysShowIndicator = config.alwaysShowIndicator;
    }

    _checkLateBluezBatteryReporting() {
        if (!this.useBluezBattery || this._bluezBatterySignalConnected)
            return;

        this._bluezBatterySignalConnected = true;
        this.device.connectObject(
    ...
    _updateBluezBatteryProps() {
        if (!this.useBluezBattery || !this._dataHandler)
            return;

        const bluezBatteryLevel = this.device.battery_percentage;
        if (bluezBatteryLevel <= 0)
            return;

        const hadBattery = this.batteryPercentage > 0;
        const props = this._dataHandler.getProps();
        props.computedBatteryLevel = bluezBatteryLevel;
        props.battery1Level = bluezBatteryLevel;
        props.battery1Status = 'discharging';
        this._dataHandler.setProps(props);

        if (!hadBattery && this.indicator && this.indicatorMode === 2 &&
                this.alwaysShowIndicator) {
            this._destroyIndicator();
            this._startIndicator();
        }
    }
