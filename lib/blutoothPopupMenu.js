...
export const BluetoothDeviceItem = GObject.registerClass({
    ...
    constructor(device, client, extensionObj, supportedIcons, showBatteryPercentage, showBatteryIcon, swapIconText) {
        ...
        this._device.bind_property('connected',
            this._pairIcon, 'style_class',
            GObject.BindingFlags.SYNC_CREATE);

        this._device.bind_property_full('connected',
            this._pairIcon, 'style_class',
            GObject.BindingFlags.SYNC_CREATE,
            (bind, source) => [true, source ? 'shell-link' : 'popup-menu-icon'],
            null);
    ...
    _displayBatteryLevelTextPercentage() {
        if (this._showBatteryPercentage && this._iconSupported) {
            ...
            // Fix for st_widget_get_theme_node called on the widget...which is not in the stage. Credits: GdH
            this._batteryPercentageLabel.connectObject('notify::width', () => {
                const width = this._batteryPercentageLabel.get_width();
                this._batteryPercentageLabel.disconnectObject(this);
                this._batteryPercentageLabel.set_width(width);
                this._bindLabel();
            }, this);
        }
    }
