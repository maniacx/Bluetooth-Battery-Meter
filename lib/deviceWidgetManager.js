...
export const DeviceWidgetManager = GObject.registerClass({
    ...
    _aliasUpdated(alias) {
        ...
        if (this._toggle.trayManager)
            this._messageWidget?.updateAlias(alias);
    }
    ...
    _startExtendedGUI() {
        if (this._dataHandler && this._device.connected) {
            ...
            if (this._toggle.trayManager && !this._isUnlockSession) {
                this._messageWidget =
                    this._toggle.trayManager.addDevice(this._path, this._alias, this._dataHandler);
            }
    ...
    _destroyOnDisconnect() {
        ...
        this._toggle?.trayManager?.removeDevice(this._path);
