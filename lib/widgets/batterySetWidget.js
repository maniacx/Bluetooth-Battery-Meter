...
class BatterySetWidget extends St.BoxLayout {
    _init(batteryIconSize, widgetInfo, textBelowIcon, dataHandler, params = {}) {
        super._init({...params});
        this._dataHandler = dataHandler;
        this._config = this._dataHandler.getConfig();
        const orientation = textBelowIcon
            ? {x_align: Clutter.ActorAlign.START, ...boxLayoutProps} : {};
        const boxStyleClass = textBelowIcon
            ? 'bbm-battery-vertical-widget-box' : 'bbm-battery-horizontal-widget-box';
        const labelStyleClass = textBelowIcon
            ? 'bbm-battery-veritcal-widget-label' : 'bbm-battery-horizontal-widget-label';
