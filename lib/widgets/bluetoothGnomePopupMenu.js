...
export const BluetoothGnomePopupMenuItem = GObject.registerClass({
    ...
    _init(manager) {
        ...
        this._device.bind_property_full('connected',
            this._subtitle, 'text',
            GObject.BindingFlags.SYNC_CREATE,
            (bind, source) => [true, source ? GLib.dgettext('gnome-shell', 'Disconnect')
                : GLib.dgettext('gnome-shell', 'Connect')],
            null);
    ...
    _updateAccessibleName() {
        this.accessible_name = this._device.connected
            ? GLib.dgettext('gnome-shell', 'Disconnect %s').format(this._device.alias)
            : GLib.dgettext('gnome-shell', 'Connect to %s').format(this._device.alias);
    }
