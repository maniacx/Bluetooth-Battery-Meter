...
const ToggleButton = GObject.registerClass({
    ...
    _init(gIcon, colorInfo, iconName, toggleBtnTitle) {
        super._init({
        ...
        this._styleNoBorderRadius = 'border-radius: 0px;';
        this._styleBorderRadius4px = 'border-radius: 4px;';
        this._styleBgNoColor = 'background-color: transparent;';
    ...
    _updateStyle() {
        ...
        if (this.checked && this.hover)
            style += `${this._styleBgOpacity95} ${this._styleBorderRadius4px}`;
        else if (this.checked)
            style += `${this._styleBgOpacity90} ${this._styleBorderRadius4px}`;
        else if (this.hover)
            style += `${this._styleBgOpacity40} ${this._styleBorderRadius4px}`;
        else
            style += `${this._styleBgNoColor} ${this._styleNoBorderRadius}`;


        if (this._hasFocus)
            style += ` ${this._styleBorderRadius4px} ${this._styleBorderSolid}`;
        else
            style += this._styleBorderTransparent;

        this.set_style(style.trim());
    }
});
...
export const ToggleButtonsSet = GObject.registerClass({
    ...
    _init(gIcon, widgetInfo, isSecondSet, dataHandler) {
    ...
    _buildWidget() {
        ...
        this._button1 = new ToggleButton(this._gIcon, this._widgetInfo, this._icon1, accName1);
        hBox.add_child(this._button1);

        this._buttonSeparator1 = new St.Widget({style_class: 'bbm-button-separator'});
        hBox.add_child(this._buttonSeparator1);

        const accName2 = `${this._toggleTitle} ${this._acc2}`;
        this._button2 = new ToggleButton(this._gIcon, this._widgetInfo, this._icon2, accName2);
        hBox.add_child(this._button2);

        if (this._icon3) {
            this._buttonSeparator2 = new St.Widget({style_class: 'bbm-button-separator'});
            hBox.add_child(this._buttonSeparator2);

            const accName3 = `${this._toggleTitle} ${this._acc3}`;
            this._button3 = new ToggleButton(this._gIcon, this._widgetInfo, this._icon3, accName3);
            hBox.add_child(this._button3);
        }

        if (this._icon4) {
            this._buttonSeparator3 = new St.Widget({style_class: 'bbm-button-separator'});
            hBox.add_child(this._buttonSeparator3);

            const accName4 = `${this._toggleTitle} ${this._acc4}`;
            this._button4 = new ToggleButton(this._gIcon, this._widgetInfo, this._icon4, accName4);
