...
import {
    adjustColorLuminanceToRgba, adjustOpacityToRgba, colorToRgba, colorGreyOpacity
...
const CheckButton = GObject.registerClass({
    ...
    _init(gIcon, styleInfo, checkBtnTitle, initialChecked) {
        super._init({...boxLayoutProps, x_expand: true, style_class: 'bbm-checkbtn-boxLayout'});
        ...
        this.checkButton.child = this._box;
        ...
        bin.set_child(this.checkButton);
    ...
    updateStyle() {
        ...
        if (this.has_key_focus() && this.checkButton.checked) {
            boxStyle =
            `${this._styleInfo.borderCheckedStyle} ${
                this._styleInfo.boxHoverCheckedStyle}`;
        } else if (this.has_key_focus()) {
...
export const CheckButtonBin = GObject.registerClass({
    ...
    _init(gIcon, colorInfo, dataHandler, id) {
        ...
        const accentColor = colorToRgba(colorInfo.accentColor);
        const fgColor = colorToRgba(colorInfo.foregroundColor);
        ...
        const boxCheckedStyle = `background-color: ${accentColor};`;
        const boxHoverCheckedStyle = `background-color: ${hoverAccentColor};`;

        const iconStyle = 'color: transparent;';
        const iconCheckedStyle = `color: ${fgColor};`;
        const iconHoverCheckedStyle = `color: ${hoverAccentFgColor};`;

        const styleInfo = {
            borderStyle, borderHoverStyle, borderCheckedStyle, borderFocusStyle,
        ...
        checkLabels.forEach((label, i) => {
            ...
            if (i < checkLabels.length - 1) {
                const sep = new St.Widget({style_class: 'bbm-radio-separator'});
                hBox.add_child(sep);
            }
        });
