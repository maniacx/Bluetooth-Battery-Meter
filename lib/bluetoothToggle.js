...
import {HideBackgroundApps} from './hideBackgroundApps.js';
...
export const BluetoothBatteryMeter = GObject.registerClass({
    ...
    _init(settings, extensionPath, extuuid) {
        ...
        this._idleTimerId = GLib.idle_add(GLib.PRIORITY_LOW, () => {
            if (!Main.panel.statusArea.quickSettings._bluetooth?.quickSettingsItems?.[0]
    ...
    _startBluetoothToggle() {
        ...
        this.isBudsLinkInstalled = false;
        ...
        this.companionEnabled = this.settings.get_boolean('enable-companion');
        this._hideBackgroundAppsEnabled =
                this.settings.get_boolean('hide-budslink-background-apps');

        if (this.companionEnabled)
            this._monitorBudsLinkInstallation();
        ...
        this.settings.connectObject(
            ...
            'changed::enable-companion', () => {
                this.companionEnabled = this.settings.get_boolean('enable-companion');
                this._updateHideBackgroundAppsMode();
                if (this.companionEnabled)
                    this._monitorBudsLinkInstallation();
                else
                    this._unmonitorBudsLinkInstallation();

                this._setEnhancedDeviceMode();
                this._onActiveChanged();
            },
            'changed::hide-budslink-background-apps', () => {
                this._hideBackgroundAppsEnabled =
                        this.settings.get_boolean('hide-budslink-background-apps');
    ...
    _sync() {
        ...
        if (nConnected > 1) {
            this._bluetoothToggle.subtitle = GLib.dngettext('gnome-shell', '%d Connected',
                '%d Connected', nConnected).format(nConnected);
    ...
    _monitorBudsLinkInstallation() {
        this._appSystem = Shell.AppSystem.get_default();
        this._appSystem.connectObject('installed-changed', () => {
            this._isBudsLinkInstalled();
        },
        this);
        this._isBudsLinkInstalled();
    }

    _unmonitorBudsLinkInstallation() {
        this._appSystem?.disconnectObject(this);
        this._appSystem = null;
    }

    _isBudsLinkInstalled() {
        const app = this._appSystem.lookup_app('io.github.maniacx.BudsLink.desktop');
        this.isBudsLinkInstalled = !!app;
        this._setEnhancedDeviceMode();
    }

    _updateHideBackgroundAppsMode() {
        const hideappEnabled = this._hideBackgroundAppsEnabled && this.companionEnabled;
        if (hideappEnabled) {
            if (!this._hideBackgroundApps)
                this._hideBackgroundApps = new HideBackgroundApps();
        } else {
            this._hideBackgroundApps?.destroy();
            this._hideBackgroundApps = null;
        }
    }
