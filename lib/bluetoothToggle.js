...
import {loadDeviceList, saveDeviceList, sortDevicesByHistory} from './deviceList.js';
import {getReadyBluetoothToggle} from './quickSettings.js';
...
export const BluetoothBatteryMeter = GObject.registerClass({
    ...
    _init(settings, extensionPath, extuuid, extensionVersion) {
        super._init();
        this.settings = settings;
        this.extPath = extensionPath;
        this._extuuid = extuuid;
        this._log = createLogger('BluetoothToggle');
        this._log.release(`Initializing Bluetooth Battery Meter version=${extensionVersion}`);

        this._idleTimerId = GLib.idle_add(GLib.PRIORITY_LOW, () => {
            const bluetoothToggle = getReadyBluetoothToggle(
                Main.panel.statusArea.quickSettings);
            if (!bluetoothToggle) {
                this._log.info('Waiting for the Bluetooth Quick Settings toggle');
                return GLib.SOURCE_CONTINUE;
            }

            this._bluetoothToggle = bluetoothToggle;
            this._log.info('Bluetooth Quick Settings toggle is ready');
    ...
    _startBluetoothToggle() {
        ...
        this.onePlusBudsEnabled = this.settings.get_boolean('enable-oneplus-buds-device');
        this.experimentalOPOv1Enabled = this.settings.get_boolean('enable-experimental-opov1-device');
        ...
        this.settings.connectObject(
            ...
            'changed::enable-oneplus-buds-device', () => {
                this.onePlusBudsEnabled = this.settings.get_boolean('enable-oneplus-buds-device');
                this._setEnhancedDeviceMode();
                this._onActiveChanged();
            },
            'changed::enable-experimental-opov1-device', () => {
                this.experimentalOPOv1Enabled =
                    this.settings.get_boolean('enable-experimental-opov1-device');
                this._setEnhancedDeviceMode();
                this._onActiveChanged();
            },
            this
    ...
    _getRecencySortedDevices() {
        return sortDevicesByHistory(this._bluetoothToggle._getSortedDevices(), this.deviceList);
    }

    _pullDevicesFromGsetting() {
        this.deviceList = loadDeviceList(this.settings.get_strv('device-list'));
    }

    pushDevicesToGsetting() {
        const deviceList = saveDeviceList(this.deviceList);
