...
export const AirpodsDevice = GObject.registerClass({
    ...
    _initialize() {
        ...
        if (match) {
            ...
            this._config.albumArtIcon = this._modelData.budsIcon;
    ...
    _updateGsettingsProps() {
        ...
        if (this._awarenessSupported) {
            const caVolEnabled = this._settingsItems['ca-volume-enabled'];
            if (this._caVolEnabled !== caVolEnabled) {
                this._caVolEnabled = caVolEnabled;
                this._configureMediaController();
            }
    ...
    _configureMediaController() {
        const enableMediaController =
                this._wearDetectionMode !== 0 || this._awarenessSupported && this._caVolEnabled;
