...
export const AirpodsDevice = GObject.registerClass({
    ...
    _updateInitialValues() {
        ...
        this._inEarControlEnabled = this._settingsItems['in-ear-control-enabled'];
    }
    ...
    _updateGsettingsProps() {
        ...
        if (this._awarenessSupported) {
            const caVol = this._settingsItems['ca-volume'];
            if (this._caVolume !== caVol) {
                this._caVolume = caVol;
                this._setConversationAwarenessVolume();
            }
        }
    ...
    _configureMediaController() {
        const enableMediaController =
                this._inEarControlEnabled || this._awarenessSupported;
