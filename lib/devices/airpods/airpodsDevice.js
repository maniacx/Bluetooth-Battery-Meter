...
const AirpodsProtocol = 'L2CAP';
const AirpodsPSM = 4097;

export const AirpodsDevice = GObject.registerClass({
}, class AirpodsDevice extends GObject.Object {
    _init(settings, devicePath, config, props, updateDeviceMapCb, profileManager) {
        super._init();
    ...
    _initializeProfile() {
        ...
        if (fd === -1) {
            ...
            const profile = {
                type: 'airpods',
                uuid: AirpodsUUID,
                protocol: AirpodsProtocol,
                channelPsm: AirpodsPSM,
            };

            this._profileManager.registerProfile('airpods', profile);
        } else {
            this._startAirpodsSocket(fd);
        }
    }
    ...
    _startConfiguration(battInfo) {
        ...
        else if (!pathsString.some(device => device.path === this._devicePath))
            this._addMediaConfigToSettings();
    ...
    _addMediaConfigToSettings() {
        const item = {
            ...
            'ca-supported': this._awarenessSupported,
        ...
        if (existingPathIndex !== -1) {
            ...
            if (enableMediaController && !this._mediaController) {
                this._mediaController =
                    new MediaController(this._devicePath, this._inEarControl, this._caVolume);
    ...
    _updatetoggleVisibility() {
        ...
        if (this._inEar && this._ancSupported) {
            const showANC = true;
            const showCA = this._awarenessSupported && this._outputIsA2dp;
    ...
    updateInEarStatus(bud1Status, bud2status) {
        const inEar = bud1Status === EarDetection.IN_EAR || bud2status === EarDetection.IN_EAR;
