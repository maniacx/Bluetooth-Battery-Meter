...
import {
    GfpsUUID, DeviceTypeGfps, MessageGroup, DeviceInfoMessage, HearableControlMessage, ANCMode
} from './gfpsConfig.js';
...
export const GfpsSocket = GObject.registerClass({
    ...
    _init(devicePath, profileManager, profile, callbacks) {
        ...
        this._log = createLogger('GfpsSocket');
    ...
    async postConnectInitialization() {
        if (!this._callbacks.supportsAnc)
            return;
    ...
    _parseData(group, code, payload) {
        if (group === MessageGroup.DEVICE_INFO && code === DeviceInfoMessage.BATTERY_UPDATE) {
            ...
            if (caseInfo.status !== 'disconnected') {
                ...
                this._callbacks.saveCaseBattery?.(this._lastCaseLevel, this._lastCaseStatus);
            }
