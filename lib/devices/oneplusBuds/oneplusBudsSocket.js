...
export const OnePlusBudsSocket = GObject.registerClass({
    ...
    _dispatchPacket(packet) {
        ...
        if (events.length) {
            ...
            if (!this._ancSynchronized &&
                    (packet.command === BATTERY_RESPONSE || packet.command === STATUS_RESPONSE)) {
                // The earbuds answer ANC polls only after the initial state exchange completes.
                this._ancSynchronized = true;
                this._readNoiseMode();
            }
        }
    }
