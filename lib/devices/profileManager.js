...
export const ProfileManager = GObject.registerClass({
    ...
    async _registerProfile(deviceType, uuid, profileOptions = {}) {
        ...
        const opts = {
            ...
            AutoConnect: new GLib.Variant('b', profileOptions.autoConnect ?? true),
        };
    ...
    async acquireFd(deviceType, uuid, devicePath, profileOptions = {}) {
        if (!this._profiles.has(deviceType)) {
            ...
            const info = await this._registerProfile(deviceType, uuid, profileOptions);
            if (!info) {
                this._profiles.delete(deviceType);
                return -1;
            }
    ...
    destroy() {
        const types = Array.from(this._profiles.keys());
        for (const type of types) {
            const profile = this._profiles.get(type);
            if (profile)
                this._unregisterProfile(type, profile);
        }
    }
});
