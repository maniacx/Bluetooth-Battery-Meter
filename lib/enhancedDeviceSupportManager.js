...
import {getBluezDeviceProxy, getBluezDevicePropertiesAsync} from './bluezDeviceProxy.js';
...
import {
    OnePlusBudsDevice, DeviceTypeOnePlusBuds, isOnePlusBuds,
} from './devices/oneplusBuds/oneplusBudsDevice.js';
import {GenericOPOv1Device, DeviceTypeGenericOPOv1} from './devices/opov1/genericOpov1Device.js';
import {isGenericOPOv1} from './devices/opov1/opov1Detector.js';
...
export const EnhancedDeviceSupportManager = GObject.registerClass({
    ...
    _init(toggle) {
        ...
        if (toggle.onePlusBudsEnabled || toggle.experimentalOPOv1Enabled)
            this._discoverOPOv1Devices();
    }
    ...
    _discoverOPOv1Devices() {
        Gio.DBus.system.call(
            'org.bluez', '/', 'org.freedesktop.DBus.ObjectManager', 'GetManagedObjects',
            null, null, Gio.DBusCallFlags.NONE, -1, null,
            (connection, result) => {
                try {
                    const [objects] = connection.call_finish(result).deepUnpack();
                    for (const [path, interfaces] of Object.entries(objects)) {
                        ...
                        const properties = Object.fromEntries(
                            Object.entries(device).map(([key, value]) => [key, value.deepUnpack()])
                        );
                        const isConfirmed = isOnePlusBuds(properties, []).supported === 'yes';
                        const isGeneric = isGenericOPOv1(properties, properties.UUIDs ?? []).supported === 'yes';
                        if ((isConfirmed && this._toggle.onePlusBudsEnabled) ||
                                (isGeneric && this._toggle.experimentalOPOv1Enabled))
                            this._rememberOnePlusBudsDevice(path, properties.Alias ?? properties.Name,
                                isConfirmed ? 'oneplus-buds-pro-3' : 'generic-opov1');
                    }
                } catch (error) {
                    this._log.error(error, 'Cannot scan BlueZ devices for OnePlus/Oppo Buds');
                }
            }
        );
    }
    ...
    onDeviceSync(path, connected, icon, alias) {
        ...
        if (deviceProps.pendingDetection) {
            if (deviceProps.bluezLoading)
                return {
                    type: deviceProps.type, dataHandler: deviceProps.dataHandler,
                    pendingDetection: deviceProps.pendingDetection,
                };

            deviceProps.bluezLoading = true;
            getBluezDevicePropertiesAsync(path, (bluezDeviceProps, error) => {
                deviceProps.bluezLoading = false;
                if (error) {
                    this._log.error(error,
                        `Cannot load BlueZ properties path=${sanitizeDevPath(path)}`);
                    return;
                }
                if (this._deviceMap.has(path)) {
                    this._detectDevice(path, deviceProps, bluezDeviceProps);
                    this._toggle.sync();
                }
            });
            return {
    ...
    _detectDevice(path, deviceProps, bluezDeviceProps) {
        const uuids = bluezDeviceProps.UUIDs ?? [];

        /* ----- Add device variant here _______ */
        const deviceModes = [
                {
                    enabled: this._toggle.onePlusBudsEnabled,
                    check: isOnePlusBuds,
                    type: DeviceTypeOnePlusBuds,
                },
                {
                    enabled: this._toggle.experimentalOPOv1Enabled,
                    check: isGenericOPOv1,
                    type: DeviceTypeGenericOPOv1,
                },
                {
        ...
        for (const mode of deviceModes) {
            ...
            const {supported, bluezProps} = mode.check(bluezDeviceProps, uuids);
                ...
                if (supported === 'pending') {
                    ...
                    this._waitForBluezProps(path, getBluezDeviceProxy(path), bluezProps, deviceProps);
                    break;
                }
                ...
                if (supported === 'yes') {
                    ...
                    if (mode.type === DeviceTypeOnePlusBuds || mode.type === DeviceTypeGenericOPOv1)
                        this._rememberOnePlusBudsDevice(path, deviceProps.alias,
                            mode.type === DeviceTypeOnePlusBuds ?
                                'oneplus-buds-pro-3' : 'generic-opov1');
                    break;
                }
                deviceProps.pendingDetection = false;
        }
    ...
    _rememberOnePlusBudsDevice(path, alias, profile) {
        const key = 'oneplus-buds-list';
        const devices = this._settings.get_strv(key).map(JSON.parse);
        if (devices.some(device => device.path === path))
            return;

        // Persist detection independently from RFCOMM readiness for per-device preferences.
        devices.push({
            path,
            alias,
            icon: 'earbuds-stem',
        ...
        this._settings.set_strv(key, devices.map(device => JSON.stringify(device)));
    }

    _waitForBluezProps(path, bluezDeviceProxy, bluezProps, deviceProps) {
        if (deviceProps.bluezId !== null && deviceProps.bluezDeviceProxy)
            return;
        ...
        const completeDetection = () => {
            ...
            if (this._deviceMap.has(path)) {
                ...
                props.bluezDeviceProxy.disconnect(props.bluezId);
                props.bluezId = null;
        ...
        // D-Bus may finish the initial property load before signal subscription.
        GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
            if (deviceProps.bluezId !== null && allPropsReady())
                completeDetection();
    ...
    updateEnhancedDevicesInstance() {
        for (const [path, deviceProps] of this._deviceMap.entries()) {
            if (deviceProps.type && deviceProps.connected && !deviceProps.enhancedDevice) {
                ...
                } else if (deviceProps.type === DeviceTypeOnePlusBuds) {
                    deviceProps.enhancedDevice =
                        new OnePlusBudsDevice(this._settings, path, deviceProps.alias, this._extPath,
                            this._profileManager, this.updateDeviceMapCb.bind(this));
                } else if (deviceProps.type === DeviceTypeGenericOPOv1) {
                    deviceProps.enhancedDevice =
                        new GenericOPOv1Device(this._settings, path, deviceProps.alias, this._extPath,
                            this._profileManager, this.updateDeviceMapCb.bind(this));
