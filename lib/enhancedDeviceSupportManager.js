...
import {
    RealmeBudsDevice, isRealmeBuds, DeviceTypeRealmeBuds
} from './devices/realmeBuds/realmeBudsDevice.js';
import {GoogleBudsDevice, isGoogleBuds, DeviceTypeGoogleBuds} from './devices/googleBuds/googleBudsDevice.js';
import {GfpsDevice, isGfps, DeviceTypeGfpsStr} from './devices/gfps/gfpsDevice.js';
...
export const EnhancedDeviceSupportManager = GObject.registerClass({
    ...
    onDeviceSync(path, connected, icon, alias) {
        ...
        if (deviceProps.pendingDetection) {
            ...
            const deviceModes = [
                ...
                {
                    enabled: this._toggle.realmeBudsEnabled,
                    check: isRealmeBuds,
                ...
                if (deviceProps.type === DeviceTypeAirpods) {
                    ...
                    deviceProps.enhancedDevice =
                        new NothingBudsDevice(this._settings, path, deviceProps.alias,
                            this._extPath, this._profileManager,
                            this.updateDeviceMapCb.bind(this));
                } else if (deviceProps.type === DeviceTypeRealmeBuds) {
                    deviceProps.enhancedDevice =
                        new RealmeBudsDevice(this._settings, path, deviceProps.alias,
                            this._extPath, this._profileManager,
                            this.updateDeviceMapCb.bind(this));
                    ...
                    deviceProps.enhancedDevice =
                        new GoogleBudsDevice(this._settings, path, deviceProps.alias,
                            this._extPath, this._profileManager,
                            this.updateDeviceMapCb.bind(this));
                } else if (deviceProps.type === DeviceTypeGfpsStr) {
                    deviceProps.enhancedDevice =
                        new GfpsDevice(this._settings, path, deviceProps.alias,
                            this._extPath, this._profileManager,
                            this.updateDeviceMapCb.bind(this),
                            deviceProps.lastCaseLevel || 0,
                            deviceProps.lastCaseStatus || 'disconnected',
                            this.updateCaseBatteryCb.bind(this));
    ...
    updateCaseBatteryCb(devicePath, level, status) {
        if (this._deviceMap.has(devicePath)) {
            const deviceProps = this._deviceMap.get(devicePath);
            deviceProps.lastCaseLevel = level;
            deviceProps.lastCaseStatus = status;
        }
    }
