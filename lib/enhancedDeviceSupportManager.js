...
import {AirpodsDevice, AirpodsUUID} from './airpods/airpodsDevice.js';
import {GattBasDevices, GattBasUUID} from './gattBas/gattBasDevices.js';

const SCRIPT_VERSION = 'BBM0.004';
const SERVICE_STANDALONE_TEST = true;
...
export const EnhancedDeviceSupportManager = GObject.registerClass({
    ...
    async _restartScript() {
        ...
        if (this._versionDiffCount > 0)
            ...
            console.log('Bluetooth-Battery-Meter: Script Version not compatible');
    }

    _startDbusServiceScript() {
        if (SERVICE_STANDALONE_TEST)
            return;
        ...
        const serviceDir = `${this._extensionPath}/service`;
        ...
        const mainScriptPath = `${serviceDir}/service.js`;
        const command = `gjs -m ${mainScriptPath}`;
