...
export const EnhancedDeviceSupportManager = GObject.registerClass({
    ...
    _init(toggle) {
        ...
        if (toggle.onePlusBudsEnabled)
            this._discoverOnePlusBuds();
    }
            ...
            (connection, result) => {
                try {
                    ...
                    for (const [path, interfaces] of Object.entries(objects)) {
                        ...
                        if (isOnePlusBuds(properties, []).supported === 'yes')
                            this._rememberOnePlusBudsDevice(path, properties.Alias ?? properties.Name);
                    }
                } catch (error) {
                    this._log.error(error, 'Cannot scan BlueZ devices for OnePlus Buds Pro 3');
                }
            }
        );
    }
        ...
        for (const mode of deviceModes) {
            ...
            const {supported, bluezProps} = mode.check(bluezDeviceProps, uuids);
                ...
                if (supported === 'yes') {
                    ...
                    if (mode.type === DeviceTypeOnePlusBuds)
    ...
    _rememberOnePlusBudsDevice(path, alias) {
