...
import {AirpodsModelList} from './airpodsHelper.js';
...
export const AirpodsDevice = GObject.registerClass({
            ...
            this._deviceProxy.connectObject(
                'g-properties-changed', (proxy, changedProps) => {
                    if (this._battInfoRecieved) {
                        ...
                        if (this._conversationAwarenessSupported &&
                                        !!changedProps.lookup_value('ToggleVisible', null))
                            this._updateToggleVisible();
    ...
    _startConfiguration() {
        ...
        if (this._ancSupported) {
            ...
            this._props.toggleVisible =
                this._deviceProxy.get_cached_property('ToggleVisible').unpack();
    ...
    _updateToggleVisible() {
        if (!this._deviceProxy)
            return;
        const toggleVisible =
            this._deviceProxy.get_cached_property('ToggleVisible').unpack();

        if (!this._outputIsA2dp && toggleVisible === 0)
            this._props.toggleVisible = 2;
        else if (!this._outputIsA2dp && toggleVisible === 1)
            this._props.toggleVisible = 3;
        else
    ...
    _updateCustomMessage() {
        ...
        if (bud2InearStatus && bud2InearStatus) {
            if (bud1InearStatus === 'in-ear' || bud2InearStatus === 'in-ear')
