...
export async function initProxy(cancellable) {
    try {
        ...
        log('Bluetooth-Battery-Meter: Failed to create DBus proxy');
        return null;
    }
}
...
export async function initDeviceProxy(path, cancellable, onPropertiesChangedCallback) {
    try {
        ...
        log(`Bluetooth-Battery-Meter: Failed to create DBus proxy for ${path}`);
        return [null, null];
    }
}
...
export async function getDeviceProps(path, cancellable, requestedProps) {
    try {
        ...
        log(`Bluetooth-Battery-Meter: Could not get properties for ${path}`);
        return null;
    }
}
...
export async function getDevices(dbusProxy, cancellable, requestedProps) {
    try {
        ...
        log('Bluetooth-Battery-Meter: Could not EnumerateDevices');
    }
    return [];
}
