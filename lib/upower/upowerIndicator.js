...
const UpowerDevice = GObject.registerClass({
    ...
    _sync() {
        ...
        const showIndicator =
            indicatorProp.indicatorMode && isPresent &&
            (!indicatorProp.reportsState || isValidBatteryState);

        if (showIndicator && !indicator) {
            indicator = new BluetoothIndicator(this, 2, indicatorProp.icon);
            this._upowerClientObj.addUPIndicatorWidget(indicator);
...
export const UpowerClient = GObject.registerClass({
    ...
    constructor(settings, widgetInfo, addIndicatorWidget) {
        super();
        this._settings = settings;
        this._widgetInfo = widgetInfo;
        this._addIndicatorWidget = addIndicatorWidget;
    ...
    _addUPIndicatorBoxLayout() {
        this._upIndicatorBox = new St.BoxLayout();
        this._addIndicatorWidget(this._upIndicatorBox);
        const removedSignal = shellVersion > 45 ? 'child-removed' : 'actor-removed';
        this._upIndicatorBox.connectObject(
            removedSignal, () => {
                if (this._upIndicatorBox.get_n_children() === 0)
                    this._removeUPIndicatorBoxLayout();
            },
            this
        );
    }

    _removeUPIndicatorBoxLayout() {
        this._upIndicatorBox?.disconnectObject(this);
        this._upIndicatorBox?.destroy();
        this._upIndicatorBox = null;
    }

    addUPIndicatorWidget(widget) {
        if (!this._upIndicatorBox)
            this._addUPIndicatorBoxLayout();
        this._upIndicatorBox.add_child(widget);
    }
