...
const UpowerDevice = GObject.registerClass({
    ...
    _sync() {
        ...
        const deviceList = this._upowerClientObj._deviceList;
        const pathProps = deviceList.get(this._path);

        // UPower can reassign an object path to a different physical device
        // across sessions, or move a device to a path that's never been used
        // before (e.g. hidpp_battery_1 disappearing and reappearing as
        // hidpp_battery_3). Only trust the entry already at our own path if
        // it's actually ours (matching model, or no model to compare against
        // yet). Otherwise look for a previously persisted entry with a
        // matching model anywhere in the list and adopt it instead, swapping
        // whatever was at our own path into its old slot so the device it
        // actually belongs to can find it in turn.
        let matchedPath = null;
        if (pathProps && (!model || pathProps.model === model)) {
            matchedPath = this._path;
        } else if (model) {
            for (const [existingPath, existingProps] of deviceList) {
                if (existingPath !== this._path && existingProps.model === model) {
        ...
        if (matchedPath && deviceList.has(matchedPath)) {
            deviceProp = deviceList.get(matchedPath);
            if (matchedPath !== this._path) {
                deviceList.delete(matchedPath);
                if (pathProps)
                    deviceList.set(matchedPath, pathProps);
        ...
        deviceList.set(this._path, deviceProp);
        let device;
