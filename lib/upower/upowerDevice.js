...
const UpowerDevice = GObject.registerClass({
    ...
    _sync() {
        ...
        // across sessions If the entry persisted under our own path reports a
        // different model than we do, our device likely moved to a new path.
        // Look for a previously persisted entry with a matching model and adopt
        // it instead of a stranger's, swapping whatever was at our own path
        // into its old slot so the device it actually belongs to can find it
        // in turn.
        let matchedPath = this._path;
        if (model && pathProps && pathProps.model && pathProps.model !== model) {
            matchedPath = null;
