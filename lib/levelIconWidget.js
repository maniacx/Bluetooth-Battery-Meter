...
var LevelIconWidget = GObject.registerClass(
class LevelIconWidget extends St.DrawingArea {
    ...
    _assignWidgetColor() {
        const colorObject = hexColor => Clutter.Color.from_string(hexColor)[1]; ;
    ...
    _addVectorImage(cr, path)  {
        Clutter.cairo_set_source_color(cr, this._colors.symbolicColor);
        cr.translate(0, 0);
        let currentX = 0;
        let currentY = 0;
    ...
    _drawBatteryBarLevel(cr) {
        ...
        Clutter.cairo_set_source_color(cr, this._colors.baseLevelColor);
        drawRoundedRect(cr, 0.5, 13.4, 14.5, 2.6, 0.15);
        if (this._percentage >= 100)
            Clutter.cairo_set_source_color(cr, this._colors.fillLevelColor);
