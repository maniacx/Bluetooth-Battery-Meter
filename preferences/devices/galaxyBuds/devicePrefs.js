...
const  DeviceItem = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_GalaxyBudsDeviceItem',
    ...
    constructor(settings, deviceItem, pathInfo) {
        ...
        this._deleteButton.connect('clicked', () => {
            ...
            if (existingPathIndex !== -1) {
                ...
                settings.set_strv('galaxy-buds-list', pairedDevices);
            }
    ...
    updateProperites(pathInfo) {
        ...
        this._deleteButton.sensitive = !this._settings.get_boolean('enable-galaxy-buds-device');
...
export const  GalaxyBuds = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_GalaxyBudsUI',
    Template: GLib.Uri.resolve_relative(
        import.meta.url, '../../../ui/devices/galaxyBuds.ui', GLib.UriFlags.NONE
    ),
    InternalChildren: [
        'enable_galaxy_buds_device',
        'galaxy_buds_group',
        'no_galaxy_buds_paired_row',
    ],
}, class GalaxyBuds extends Adw.PreferencesPage {
    constructor(settings) {
        ...
        settings.bind(
            'enable-galaxy-buds-device',
            this._enable_galaxy_buds_device,
            'active',
    ...
    _createDevices() {
        ...
        if (!pathsString || pathsString.length === 0) {
            this._no_galaxy_buds_paired_row.visible  = true;
            return;
        }
        this._no_galaxy_buds_paired_row.visible  = false;
