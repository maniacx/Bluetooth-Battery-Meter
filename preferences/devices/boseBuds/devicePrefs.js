...
const  DeviceItem = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_BoseBudsDeviceItem',
    ...
    constructor(settings, deviceItem, pathInfo) {
        ...
        this._deleteButton.connect('clicked', () => {
            const pairedDevices = settings.get_strv('bose-buds-list');
            ...
            if (existingPathIndex !== -1) {
                ...
                settings.set_strv('bose-buds-list', pairedDevices);
            }
    ...
    updateProperites(pathInfo) {
        ...
        this._deleteButton.sensitive = !this._settings.get_boolean('enable-bose-buds-device');
...
export const  BoseBuds = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_BoseBudsUI',
    Template: GLib.Uri.resolve_relative(
        import.meta.url, '../../../ui/devices/boseBuds.ui', GLib.UriFlags.NONE
    ),
    InternalChildren: [
        'enable_bose_buds_device',
        'bose_buds_group',
        'no_bose_buds_paired_row',
    ],
}, class BoseBuds extends Adw.PreferencesPage {
    constructor(settings) {
        ...
        settings.bind(
            'enable-bose-buds-device',
            this._enable_bose_buds_device,
            'active',
            Gio.SettingsBindFlags.DEFAULT
        );

        this._createDevices();
        this._settingSignalId = this._settings.connect('changed::enable-bose-buds-device', () =>
            this._createDevices());

        this._settingSignalId2 = this._settings.connect('changed::bose-buds-list', () =>
            this._createDevices());
    }

    _createDevices() {
        const pathsString = this._settings.get_strv('bose-buds-list').map(JSON.parse);
        if (!pathsString || pathsString.length === 0) {
            this._no_bose_buds_paired_row.visible  = true;
            return;
        }
        this._no_bose_buds_paired_row.visible  = false;
        for (const info of pathsString) {
            ...
            if (this._deviceItems.has(pathInfo.path)) {
                ...
                this._bose_buds_group.add(deviceItem);
            }
        }
    }
