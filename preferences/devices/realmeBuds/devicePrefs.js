...
const DeviceItem = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_RealmeBudsDeviceItem',
    ...
    constructor(settings, deviceItem, pathInfo) {
        ...
        this._deleteButton.connect('clicked', () => {
            ...
            if (existingPathIndex !== -1) {
                ...
                settings.set_strv('realme-buds-list', pairedDevices);
            }
    ...
    updateProperites(pathInfo) {
        ...
        this._deleteButton.sensitive = !this._settings.get_boolean('enable-realme-buds-device');
...
export const RealmeBuds = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_RealmeBudsUI',
    Template: GLib.Uri.resolve_relative(
        import.meta.url, '../../../ui/devices/realmeBuds.ui', GLib.UriFlags.NONE
    ),
    InternalChildren: [
        'enable_realme_buds_device',
        'realme_buds_group',
        'no_realme_buds_paired_row',
    ],
}, class RealmeBuds extends Adw.PreferencesPage {
    constructor(settings) {
        ...
        settings.bind(
            'enable-realme-buds-device',
            this._enable_realme_buds_device,
            'active',
            Gio.SettingsBindFlags.DEFAULT
        );

        this._createDevices();
        this._settings.connect('changed::enable-realme-buds-device', () => this._createDevices());
        this._settings.connect('changed::realme-buds-list', () => this._createDevices());
    }

    _createDevices() {
        const pathsString = this._settings.get_strv('realme-buds-list').map(JSON.parse);
        if (!pathsString || pathsString.length === 0) {
            this._no_realme_buds_paired_row.visible = true;
            return;
        }
        this._no_realme_buds_paired_row.visible = false;
        for (const info of pathsString) {
            ...
            if (this._deviceItems.has(pathInfo.path)) {
                ...
                this._realme_buds_group.add(deviceItem);
            }
        }
    }
});
