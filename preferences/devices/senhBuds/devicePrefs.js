...
const  DeviceItem = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_SenhBudsDeviceItem',
    ...
    constructor(settings, deviceItem, pathInfo) {
        ...
        this._deleteButton.connect('clicked', () => {
            const pairedDevices = settings.get_strv('senh-buds-list');
            ...
            if (existingPathIndex !== -1) {
                ...
                settings.set_strv('senh-buds-list', pairedDevices);
            }
    ...
    updateProperites(pathInfo) {
        ...
        this._deleteButton.sensitive = !this._settings.get_boolean('enable-senh-buds-device');
...
export const  SenhBuds = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_SenhBudsUI',
    Template: GLib.Uri.resolve_relative(
        import.meta.url, '../../../ui/devices/senhBuds.ui', GLib.UriFlags.NONE
    ),
    InternalChildren: [
        'enable_senh_buds_device',
        'senh_buds_group',
        'no_senh_buds_paired_row',
    ],
}, class SenhBuds extends Adw.PreferencesPage {
    constructor(settings) {
        ...
        settings.bind(
            'enable-senh-buds-device',
            this._enable_senh_buds_device,
            'active',
            Gio.SettingsBindFlags.DEFAULT
        );

        this._createDevices();
        this._settingSignalId = this._settings.connect('changed::enable-senh-buds-device', () =>
            this._createDevices());

        this._settingSignalId2 = this._settings.connect('changed::senh-buds-list', () =>
            this._createDevices());
    }

    _createDevices() {
        const pathsString = this._settings.get_strv('senh-buds-list').map(JSON.parse);
        if (!pathsString || pathsString.length === 0) {
            this._no_senh_buds_paired_row.visible  = true;
            return;
        }
        this._no_senh_buds_paired_row.visible  = false;
        for (const info of pathsString) {
            ...
            if (this._deviceItems.has(pathInfo.path)) {
                ...
                this._senh_buds_group.add(deviceItem);
            }
        }
    }
