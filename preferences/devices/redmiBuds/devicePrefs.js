...
const  DeviceItem = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_RedmiBudsDeviceItem',
    ...
    constructor(settings, deviceItem, pathInfo) {
        ...
        this._deleteButton.connect('clicked', () => {
            const pairedDevices = settings.get_strv('redmi-buds-list');
            ...
            if (existingPathIndex !== -1) {
                ...
                settings.set_strv('redmi-buds-list', pairedDevices);
            }
    ...
    updateProperites(pathInfo) {
        ...
        this._deleteButton.sensitive = !this._settings.get_boolean('enable-redmi-buds-device');
...
export const  RedmiBuds = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_RedmiBudsUI',
    Template: GLib.Uri.resolve_relative(
        import.meta.url, '../../../ui/devices/redmiBuds.ui', GLib.UriFlags.NONE
    ),
    InternalChildren: [
        'enable_redmi_buds_device',
        'redmi_buds_group',
        'no_redmi_buds_paired_row',
    ],
}, class RedmiBuds extends Adw.PreferencesPage {
    constructor(settings) {
        ...
        settings.bind(
            'enable-redmi-buds-device',
            this._enable_redmi_buds_device,
            'active',
            Gio.SettingsBindFlags.DEFAULT
        );

        this._createDevices();
        this._settingSignalId = this._settings.connect('changed::enable-redmi-buds-device', () =>
            this._createDevices());

        this._settingSignalId2 = this._settings.connect('changed::redmi-buds-list', () =>
            this._createDevices());
    }

    _createDevices() {
        const pathsString = this._settings.get_strv('redmi-buds-list').map(JSON.parse);
        if (!pathsString || pathsString.length === 0) {
            this._no_redmi_buds_paired_row.visible  = true;
            return;
        }
        this._no_redmi_buds_paired_row.visible  = false;
        for (const info of pathsString) {
            ...
            if (this._deviceItems.has(pathInfo.path)) {
                ...
                this._redmi_buds_group.add(deviceItem);
            }
        }
    }
