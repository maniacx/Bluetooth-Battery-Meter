...
const DeviceItem = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_PixelBudsDeviceItem',
...
export const PixelBuds = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_PixelBudsUI',
    Template: GLib.Uri.resolve_relative(
        import.meta.url, '../../../ui/devices/pixelBuds.ui', GLib.UriFlags.NONE
    ),
    InternalChildren: [
        'enable_pixel_buds_device',
        'pixel_buds_group',
        'no_pixel_buds_paired_row',
    ],
}, class PixelBuds extends Adw.PreferencesPage {
    constructor(settings) {
        ...
        settings.bind(
            ...
            this._enable_pixel_buds_device,
            'active',
    ...
    _createDevices() {
        ...
        if (!pathsString || pathsString.length === 0) {
            this._no_pixel_buds_paired_row.visible = true;
            return;
        }
        this._no_pixel_buds_paired_row.visible = false;
        for (const info of pathsString) {
            ...
            if (this._deviceItems.has(pathInfo.path)) {
                ...
                this._pixel_buds_group.add(deviceItem);
            }
        }
    }
});
