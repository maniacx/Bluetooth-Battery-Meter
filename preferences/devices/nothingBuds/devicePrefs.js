...
const  DeviceItem = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_NothingBudsDeviceItem',
    ...
    constructor(settings, deviceItem, pathInfo) {
        ...
        this._deleteButton.connect('clicked', () => {
            ...
            if (existingPathIndex !== -1) {
                ...
                settings.set_strv('nothing-buds-list', pairedDevices);
            }
    ...
    updateProperites(pathInfo) {
        ...
        this._deleteButton.sensitive = !this._settings.get_boolean('enable-nothing-buds-device');
...
export const  NothingBuds = GObject.registerClass({
    GTypeName: 'BluetoothBatteryMeter_NothingBudsUI',
    Template: GLib.Uri.resolve_relative(
        import.meta.url, '../../../ui/devices/nothingBuds.ui', GLib.UriFlags.NONE
    ),
    InternalChildren: [
        'enable_nothing_buds_device',
        'nothing_buds_group',
        'no_nothing_buds_paired_row',
    ],
}, class NothingBuds extends Adw.PreferencesPage {
    constructor(settings) {
        ...
        settings.bind(
            'enable-nothing-buds-device',
            this._enable_nothing_buds_device,
            'active',
    ...
    _createDevices() {
        const pathsString = this._settings.get_strv('nothing-buds-list').map(JSON.parse);
        if (!pathsString || pathsString.length === 0) {
            this._no_nothing_buds_paired_row.visible  = true;
            return;
        }
        this._no_nothing_buds_paired_row.visible  = false;
