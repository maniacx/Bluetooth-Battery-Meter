...
import {Bat1Group, Bat2Group, CpfNames} from './configGattBas.js';
...
export class GattBasDevice {
    constructor(devicePath, bluezDeviceProxy, callbacks) {
        ...
        this._bluezSignalSubId = null;
        this._batteryInit = false;
        this._charInfo = {};
        this._battProps = {};

        if (this._bluezDeviceProxy.ServicesResolved) {
    ...
    async _initBatteryCharacteristics() {
        this._batteryInit = true;
        if (this._bluezSignalSubId && this._bluezDeviceProxy)
            this._bluezDeviceProxy.disconnect(this._bluezSignalSubId);
        ...
        try {
            let objectManager;
            try {
                objectManager = await Gio.DBusProxy.new(
                    this._connection,
            ...
            for (const [path, ifaces] of Object.entries(managed)) {
                ...
                try {
                    for (const [dpath, descIfaces] of Object.entries(managed)) {
                        ...
                        const descProxy = await Gio.DBusProxy.new(
                            this._connection, Gio.DBusProxyFlags.NONE, null,
